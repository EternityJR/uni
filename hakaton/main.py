...
def main():
    ...
    form = ConversationHandler(
        entry_points=[MessageHandler(Filters.regex("^(Создать анкету)$"), form_start),
                      MessageHandler(Filters.regex("^(Изменить анкету)$"), form_edit),
    ...
    disp.add_handler(MessageHandler(Filters.regex("^(Изменить анкету)$"), form_edit))
    disp.add_handler(CommandHandler("edit_anketa", form_edit))
